...
trait Frontend_Checkout_Trait {
  ...
  public function update_order_meta_pickup_point_field( $order_id ) {
    ...
    if ( ! empty($pickup_point) ) {
      ...
      if ( ! $order ) {
        $logger->error('The order could not be received');
        return;
      }
      $order->update_meta_data('_' . str_replace('wc_', '', $this->core->prefix) . '_pickup_point', sanitize_text_field($pickup_point));
  ...
  public function pickup_point_field_html() {
    ...
    if ( empty($shipping_postcode) ) {
      ...
      if ( $show_pickup_point_override_query === 'yes' && ($selected_point === 'other' || $is_klarna || ! $options_array) ) {
        $custom_field_title = $is_klarna ? __('Pickup address', 'woo-pakettikauppa') : __('Custom pickup address', 'woo-pakettikauppa');
        ...
        $custom_field_desc = ($is_klarna) ? __('Search pickup points near you by typing your address above.', 'woo-pakettikauppa') : __('If none of your preferred pickup points are listed, fill in a custom address above and select another pickup point.', 'woo-pakettikauppa');
      }
    }
